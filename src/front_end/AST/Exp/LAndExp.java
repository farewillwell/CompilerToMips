...
public class LAndExp extends Node {
    ...
    public void getLAndIr(BasicBlock thenBlock, BasicBlock elseBlock) {
        ...
        for (int i = 0; i < eqExps.size(); i++) {
            if (i == eqExps.size() - 1) {
                BranchIr branchIr = new BranchIr(eqExps.get(i).getIRCode(), thenBlock, elseBlock);
                IRBuilder.IB.addInstrForBlock(branchIr);
            } else {
                BasicBlock nextBlock = new BasicBlock();
                BranchIr branchIr = new BranchIr(eqExps.get(i).getIRCode(), nextBlock, elseBlock);
                IRBuilder.IB.addInstrForBlock(branchIr);
