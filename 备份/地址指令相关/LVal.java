...
public class LVal extends Node {
    ...
    public Value getIRCode() {
        ...
        /*
        if (varStorePointer instanceof GlobalVar) {
            ArrayList<Value> offsets = new ArrayList<>();
            for (Exp exp : exps) {
                offsets.add(exp.getIRCode());
            }
            LoadInstr loadInstr = new LoadInstr((GlobalVar) varStorePointer, offsets);
            IRBuilder.IB.addInstrForBlock(loadInstr);
            return loadInstr.getAns();
        } else {*/
        LocalVar nowArrayOrAnsPointer = (LocalVar) varStorePointer;
        for (Exp exp : exps) {
