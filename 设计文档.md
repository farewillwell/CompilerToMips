...
这里先按照全局与否都一样（全局的话会存在压缩，但是局部好像没有压缩）
...
这个取出来的变量的类型:`inbounds [10 x i32]`

而符号表中这个名字的类型：`[10 x i32]* %2`

注意，在符号表里，名字都是一个对象的指针，或者i32对象，或者【10】对象，那么这个名字对应的就是一个[10]的指针，就是*[10]

然后根据后边的偏移计算出来要取这个对象组里的第几个对象（比如是数组中的第一个元素，就是0了）
...
因此我们只需要放一个i32 0占位就好了。（如果不写0的话，比如1，那么移动的距离是这个数组的大小）
...
显然，gep是可以满足多维数组一次取用的，所以如果有必要，需要设置多种方法。
